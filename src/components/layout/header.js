    ...
    <div className="container flex justify-between py-2 fixed rounded-b-2xl bg-black-shadow top-0 left-0 z-10 left-1/2 transform -translate-x-1/2">
      ...
      <nav className="hidden md:block">
        <ul className="flex font-inter">
          ...
          <li>
            <a role="button" onClick={()=>goToElement("about")} className="text-white py-2 px-4 block cursor-pointer hover:text-green">Suscripción</a>
          </li>
          <li>
            <a role="button" onClick={()=>goToElement("faq")} className="text-white py-2 px-4 block cursor-pointer hover:text-green">FAQ</a>
          </li>
          <li>
            <a href="https://rkeuh727s75.typeform.com/to/t5bRgwDK" target={"_blank"} className="py-2 px-4 block bg-green text-black rounded-xl cursor-pointer">Pide tu auto</a>
          </li>
        </ul>
      </nav>
    </div>
